...
if (!baseURL && process.env.CI) {
  throw new Error("HQBASE_STAGING_URL is required. Webmail E2E runs only in staging.");
}
if (baseURL && process.env.CI && (!accessClientId || !accessClientSecret)) {
  throw new Error(
    "Cloudflare Access service-token credentials are required for Webmail staging E2E."
  );
}
