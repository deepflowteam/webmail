...
export function resolveOAuthConfig(
  ...
  if (new URL(request.url).origin !== canonicalOrigin) {
    throw invalidOAuthConfig(
      "Open the canonical Webmail URL before authorizing the customer-managed OAuth client."
    );
  }
  return {
...
function customerOAuthOrigin(value: string | undefined): string {
  if (!value) {
    throw invalidOAuthConfig(
      "Customer-managed OAuth requires BETTER_AUTH_URL to be the canonical HTTPS Webmail origin."
    );
  }
  let url: URL;
  try {
